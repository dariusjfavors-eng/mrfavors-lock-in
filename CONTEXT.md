...
- 2026-05-25 (Deploy fix) — Discovered live site serves from `mrfavors-lock-in/main`, not `origin/gh-pages`. Sessions S–V all pushed to `origin/gh-pages` (mrfavors-algebra repo — wrong remote); live site was running Session R-era code. Fixed: pushed Session X subtree to `mrfavors-lock-in main`. Correct deploy command: `git subtree push --prefix games/regents-mc-trainer mrfavors-lock-in main`. DO NOT use `origin gh-pages` for live deploys.
- 2026-05-15 (Session J — Deployment) — main pushed (8 commits, Sessions I–K). `git subtree push --prefix games/regents-mc-trainer origin gh-pages` created gh-pages branch on mrfavors-algebra (NOTE: this is NOT the live site — see deploy fix note above). GitHub Pages on mrfavors-lock-in enabled → `https://dariusjfavors-eng.github.io/mrfavors-lock-in/` (serves from mrfavors-lock-in/main). COMMAND_CENTER.md updated: Build → Classroom Ready.
